...
const API_BASE_URL = 'http://localhost:5001/api/gate-pass'; // Backend API base URL for gate pass
...
// Add a new gate pass entry
export const addGatePassEntry = async (entryData) => {
  // entryData should be an object like: { date, styleNumber, destination, quantity }
  try {
...
export const getPendingGatePasses = async () => {
  ...
  } catch (error) { console.error('Error fetching pending gate passes:', error); throw error; }
};
...
export const approveGatePass = async (id) => {
   try {
    ...
    return await response.json();
  } catch (error) { console.error(`Error approving gate pass ${id}:`, error); throw error; }
};
...
export const rejectGatePass = async (id) => {
   try {
    ...
    return await response.json();
  } catch (error) { console.error(`Error rejecting gate pass ${id}:`, error); throw error; }
};